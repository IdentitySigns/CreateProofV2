...
//     }
// }
const createProofHtml = (data, previewImageSrc) => {
    var _a, _b, _c, _d;
    let listOfMaterialSpecifications = data.itemInfo.itemMaterialSpecifications.map((s) => s.variableName !== 'Machine');
    return `
    <!doctype html>
    <html lang=en>
    <head>
    ...
    <body>
        ...
        <div class=wo_f_bt_col_body>
        ${listOfMaterialSpecifications.map((cc) => (`
          <div class=wo_f_bt_col_body_img_cont id="${cc.variableName}">
          <img class=wo_f_bt_col_body_img src="https://identitysigns-x3-fai8o.your-cloudlab.com/media/wysiwyg/IdentitySigns/WorkOrderAssets/${(0, helpers_js_3.findAssetImg)(cc.name)}" alt="${cc.name}">
        ...
        <div class=wo_f_bt_col_body>
        <p class=id_po_text>${data.orderInfo.orderDeliveryInfo.map((info) => (info.name))}</p>
        </div>
        </div>
        </div>
        </div>
        ...
        <small>${(0, helpers_js_2.formatISODateToMMDDYY)(data.orderInfo.orderCreatedDate)}</small>
        </div>
